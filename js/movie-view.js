...
var movieView = (function() {
	...
	var appendFilteredMovies = function(){
		...
		targetDiv.innerHTML = movieDatabase.currentGenres.length > 0 ? 
			...
			//movieTemplate.appendMovies(utils.sortObjectsByKey(movies, movieDatabase.getSortBy(), movieDatabase.getSortOrder()));
			movieTemplate.movieList(utils.sortObjectsByKey(movieDatabase.getMoviesByGenres(movieDatabase.currentGenres, movies), movieDatabase.getSortBy(), movieDatabase.getSortOrder()) ) : 
			movieTemplate.movieList(utils.sortObjectsByKey(movies, movieDatabase.getSortBy(), movieDatabase.getSortOrder() ) );			
		// Add event handlers
		addMovieBtnHandlers();
	...
	*/
	...
	var filterBtnOnClick = function(event){
	    ...
	    if(currentIndex > -1){
	    	removeFilterCloseButton(movieDatabase.currentGenres[currentIndex]);
	    	movieDatabase.currentGenres.splice(currentIndex, 1);
	    }else{
    		movieDatabase.currentGenres.push(this.value);	
    		appendFilterCloseButton(this.value);	
	    }    
	 ...
	 * 
	 */
	var appendFilterCloseButton = function(value){
		//console.log('Add closebuttons');	
		document.querySelector('.close-filter-buttons').insertAdjacentHTML('beforeend', movieTemplate.closeFilterBtn(value));
		document.querySelector('.close-filter-buttons')
		.querySelector(`button[value="${value}"]`)
		.addEventListener('click', filterCloseBtnOnClick, false);
	};

	/**
	 * 
	 */
	var removeFilterCloseButton = function(value){
		//console.log('Add closebuttons');	
		if(document.querySelector('.close-filter-buttons').querySelector(`button[value="${value}"]`) !== null){
			document.querySelector('.close-filter-buttons').querySelector(`button[value="${value}"]`)
			.parentNode.removeChild(document.querySelector('.close-filter-buttons').querySelector(`button[value="${value}"]`));
		}
	};
	...
	*/
	...
	var initUpdateForm = function(){
	 	...
	 	checkSelectedGenres(movieDatabase.getCurrentMovie().genres);
		var submitBtn = document.getElementById('movieFormSubmit');
